...
import com.tjcloud.tenpay.api.enums.ZjgTenpayAppId;
import com.tjcloud.tenpay.api.enums.ZjgTenpayMchApiKey;

import java.util.LinkedHashMap;
import java.util.Map;
import java.util.UUID;

public class ZjgPayUtil {
...
//		appId，partnerId，prepayId，nonceStr，timeStamp，package。注意：package的值格式为Sign=WXPay 
		...
		if(TenpayTradeType.Tenpay_JSAPI.getText().equals(tradeType)){
			package_ = "prepay_id="+prepayId;
		}else{
			package_ = "Sign=WXPay";
		}
		PayJsRequest payJsRequest = new PayJsRequest();
		payJsRequest.setAppid(ZjgTenpayAppId.getText(tradeType));
		...
		Map<String, String> mapS = (TenpayTradeType.Tenpay_JSAPI.getText().equals(tradeType))
								 ? MapUtil.jsonStringToMap(JsonUtil.toJSONString(payJsRequest))
								 : MapUtil.objectToMap(payJsRequest);
		String paySign = TenpaySignature.generateSign(mapS,ZjgTenpayMchApiKey.getText(tradeType));
		if(TenpayTradeType.Tenpay_JSAPI.getText().equals(tradeType)){
